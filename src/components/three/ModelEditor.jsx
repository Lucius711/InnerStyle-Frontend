...
export default function ModelEditor({ url, baseColorUrl, open, onClose, task, onModelUpdated, leftPanel = null }) {
  ...
  /**
   * Get the base-color texture as a Blob.
   *
   * Strategy (in order):
   * 1. Fetch baseColorUrl (same-origin backend proxy) — reliable, no CORS issues.
   * 2. Canvas-extract from the first material map found in the scene — works for uploaded
   *    models whose textures were embedded in the GLB and are therefore non-tainted.
   */
  const getTextureBlob = async (scene3d) => {
    // Strategy 1: fetch from backend proxy (works for Meshy-generated models)
    if (baseColorUrl) {
      try {
        const r = await fetch(baseColorUrl, { credentials: "include" });
        if (r.ok) return r.blob();
      } catch { /* fall through */ }
    }

    // Strategy 2: canvas-extract from scene materials (works for uploaded/embedded textures)
    let foundImage = null;
    scene3d.traverse((o) => {
      if (foundImage || !o.isMesh) return;
      const mats = Array.isArray(o.material) ? o.material : [o.material];
      for (const m of mats) {
        if (m?.map?.image) { foundImage = m.map.image; break; }
      }
    });
  ...
  /**
   * Apply a texture (blob URL) to every material in the scene that has no existing map,
   * so GLTFExporter can embed it. Returns the blob URL so the caller can revoke it.
   */
  const patchMaterialsForExport = (scene3d, blobUrl) => {
    ...
    return patched ? blobUrl : null;
  };

  /**
   * Build and download a ZIP containing the model file + texture PNG.
   * For OBJ, auto-generates an MTL whose material names match the exported OBJ.
   */
  const saveAsZip = async (modelData, modelType, ext, texBlob) => {
    const zip = new JSZip();
    ...
    if (ext === "obj" && texBlob) {
      // Parse the OBJ text to find all unique `usemtl` names, then build a matching MTL.
      const objText = await modelBlob.text();
      const usedMaterials = [...new Set(
        [...objText.matchAll(/^usemtl (.+)$/gm)].map((m) => m[1].trim())
      )];

      // Patch OBJ to reference the MTL file
      const patchedObj = `mtllib ${base}.mtl\n${objText}`;
      zip.file(`${base}.obj`, patchedObj);
    ...
    triggerDownload(zipBlob, `${base}-${ext}.zip`);
    setExporting(false);
  ...
  const exportAs = async (fmt) => {
    ...
    try {
      ...
      if (texBlobUrl && fmt === "glb") {
        patchedBlobUrl = patchMaterialsForExport(s, texBlobUrl);
      }
      ...
      if (fmt === "glb") {
        new GLTFExporter().parse(
          s,
          (res) => {
            if (texBlobUrl) URL.revokeObjectURL(texBlobUrl);
            saveAsZip(res, "model/gltf-binary", "glb", texBlob);
          },
          () => { if (texBlobUrl) URL.revokeObjectURL(texBlobUrl); fail(); },
          { binary: true, embedImages: true }
        );
      } else if (fmt === "obj") {
        await saveAsZip(new OBJExporter().parse(s), "text/plain", "obj", texBlob);
        if (texBlobUrl) URL.revokeObjectURL(texBlobUrl);
      } else if (fmt === "stl") {
        await saveAsZip(new STLExporter().parse(s, { binary: true }), "model/stl", "stl", texBlob);
        if (texBlobUrl) URL.revokeObjectURL(texBlobUrl);
      } else if (fmt === "3mf") {
        await saveAsZip(export3mf(s).data, "model/3mf", "3mf", texBlob);
        if (texBlobUrl) URL.revokeObjectURL(texBlobUrl);
      } else if (fmt === "ply") {
        new PLYExporter().parse(
          ...
          (res) => {
            if (texBlobUrl) URL.revokeObjectURL(texBlobUrl);
            saveAsZip(res, "application/octet-stream", "ply", texBlob);
          },
          { binary: true }
        );
      } else if (fmt === "usdz") {
        const usdzData = await new USDZExporter().parseAsync(s);
        if (texBlobUrl) URL.revokeObjectURL(texBlobUrl);
        await saveAsZip(usdzData, "model/vnd.usdz+zip", "usdz", texBlob);
      } else {
        if (texBlobUrl) URL.revokeObjectURL(texBlobUrl);
        fail();
      }
      ...
      // Remove the temporary blob-URL texture patch from materials
      if (patchedBlobUrl) {
        s.traverse((o) => {
          if (!o.isMesh) return;
          const mats = Array.isArray(o.material) ? o.material : [o.material];
          mats.forEach((m) => {
            if (m?.map?.image?.src === patchedBlobUrl) {
              m.map = null;
              m.needsUpdate = true;
            }
          });
